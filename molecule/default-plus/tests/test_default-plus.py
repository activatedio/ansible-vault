...
def test_vault_config(host):
    ...
    assert f.contains('tls_cert_file = "/etc/vault/vault-consul-client.crt"')
    assert f.contains('tls_key_file = "/etc/vault/vault-consul-client.key"')
    assert not f.contains('tls_ca_file')
    assert f.contains('listener "tcp" {')
    assert f.contains('tls_disable = 0')
    assert f.contains('tls_cert_file = "/etc/vault/vault-server.crt"')
    assert f.contains('tls_key_file = "/etc/vault/vault-server.key"')
